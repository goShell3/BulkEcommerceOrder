...
class NotificationService
{
    ...
    public function sendOrderCancellation(Order $order)
    {
        try {
            ...
            Log::error('Failed to send order cancellation', [
                'order_id' => $order->id,
                'error' => $e->getMessage()
            ]);
        }
    ...
    public function sendShippingConfirmation(Order $order)
    {
        try {
            ...
            Log::error('Failed to send shipping confirmation', [
                'order_id' => $order->id,
                'error' => $e->getMessage()
            ]);
        }
    ...
    public function sendReturnRequestConfirmation(ReturnRequest $returnRequest)
    {
        try {
            ...
            Log::info('Return request confirmation sent', [
                'return_request_id' => $returnRequest->id
            ]);
        } catch (\Exception $e) {
            Log::error('Failed to send return request confirmation', [
                'return_request_id' => $returnRequest->id,
                'error' => $e->getMessage()
            ]);
        }
    ...
    public function sendReturnRequestStatusUpdate(ReturnRequest $returnRequest)
    {
        try {
            ...
            Log::info('Return request status update sent', [
                'return_request_id' => $returnRequest->id,
                'status' => $returnRequest->status
            ]);
        } catch (\Exception $e) {
            Log::error('Failed to send return request status update', [
                'return_request_id' => $returnRequest->id,
                'error' => $e->getMessage()
            ]);
        }
    ...
    public function sendReturnRequestRejection(ReturnRequest $returnRequest)
    {
        try {
            ...
            Log::info('Return request rejection sent', [
                'return_request_id' => $returnRequest->id
            ]);
        } catch (\Exception $e) {
            Log::error('Failed to send return request rejection', [
                'return_request_id' => $returnRequest->id,
                'error' => $e->getMessage()
            ]);
        }
    ...
    public function sendReturnRequestCompletion(ReturnRequest $returnRequest)
    {
        try {
            ...
            Log::info('Return request completion sent', [
                'return_request_id' => $returnRequest->id
            ]);
        } catch (\Exception $e) {
            Log::error('Failed to send return request completion', [
                'return_request_id' => $returnRequest->id,
                'error' => $e->getMessage()
            ]);
        }
    ...
    public function sendReturnShippingLabel(ReturnRequest $returnRequest)
    {
        try {
            ...
            Log::info('Return shipping label sent', [
                'return_request_id' => $returnRequest->id
            ]);
        } catch (\Exception $e) {
            Log::error('Failed to send return shipping label', [
                'return_request_id' => $returnRequest->id,
                'error' => $e->getMessage()
            ]);
        }
    ...
    public function notifyAdminAboutReturnRequest(ReturnRequest $returnRequest)
    {
        try {
            ...
            Log::info('Admin notification sent for return request', [
                'return_request_id' => $returnRequest->id
            ]);
        } catch (\Exception $e) {
            Log::error('Failed to notify admin about return request', [
                'return_request_id' => $returnRequest->id,
                'error' => $e->getMessage()
            ]);
        }
